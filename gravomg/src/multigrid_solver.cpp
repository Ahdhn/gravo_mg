...
namespace MGBS {
    ...
    MultigridSolver::MultigridSolver(
        ...
        UseDense = false;
        std::cout << "*** USE DENSE IS " << UseDense << "\n";
    }
    ...
    void MultigridSolver::constructProlongation()
    {
        ...
        while (levelPoints.rows() > lowBound && k < 7) {
            ...
            mNeighborsList.push_back(neighborsList);
            //{
            //	//obj file 
            //	std::string fname("h_" + std::to_string(k + 1) + ".obj");
            //	ofstream myfile(fname);
            //	if (myfile.is_open()) {
            //		for (int i = 0; i < tempPoints.rows(); ++i) {
            //			myfile << "v " << tempPoints(i, 0) << " " << tempPoints(i, 1) << " " << tempPoints(i, 2) << "\n";
            //		}
            //
            //		for (int i = 0; i < neighborsList.size(); ++i) {
            //			for (auto it = neighborsList[i].begin(); it != neighborsList[i].end(); it++) {
            //				int j = *it;
            //				if (j < i) continue;
            //				for (auto it2 = std::next(it); it2 != neighborsList[i].end(); it2++) {
            //					int k = *it2;
            //					if (k < i) continue;
            //					if ( neighborsList[j].find(k) != neighborsList[j].end()) {
            //						myfile << "f " << i + 1 << " " << j + 1 << " " << k + 1 << "\n";
            //					}
            //				}
            //			}
            //
            ...
            if (UseDense) {
                ...
                Eigen::MatrixXd ULevelDense = Eigen::Rand::normal<Eigen::MatrixXd>(ULevel.rows(), ULevel.cols(), urng);
                for (int i = 0; i < ULevelDense.rows(); ++i) {
                    for (int j = 0; j < ULevelDense.cols(); ++j) {
                        if (ULevelDense(i, j) < 0) {
                            ULevelDense(i, j) = -1.0;
                        }
                        else {
                            ULevelDense(i, j) = 1.0;
                ...
                std::cout << ULevelDense.row(0) << "\n";
                UDense.push_back(ULevelDense);
            }
            U.push_back(ULevel);
            //if(k==1)
    ...
    double  MultigridSolver::multiGridVCycleGS(MatrixType& A, Eigen::MatrixXd& b, Eigen::MatrixXd& x, int k, bool isDebug)
    {
        ...
        //[3] Restriction
        Eigen::MatrixXd resRest;
        //Eigen::MatrixXd resRest = U[k].transpose() * res;
        if (UseDense) {
            resRest = UDense[k].transpose() * res;
        }
        else {
        ...
        if (k == U.size() - 1) {
            if (UseDense) {
                eps = coarsestSolverDense.solve(resRest);
            }
            else {
            ...
            if (UseDense) {
                multiGridVCycleGS(AbarDense[k + 1], resRest, eps, k + 1, isDebug);
            }
            else {
    ...
    void MultigridSolver::GaussSeidelSmoother(Eigen::SparseMatrix<double>& LHS, Eigen::MatrixXd& rhs, Eigen::MatrixXd& x, int maxIter, double tol, bool isDebug)
    {
        int dim = x.cols();
        if (dim == 1) {
    ...
    void MultigridSolver::GaussSeidelSmoother(Eigen::MatrixXd& LHS, Eigen::MatrixXd& rhs, Eigen::MatrixXd& x, int maxIter, double tol, bool isDebug)
    {
        int n = LHS.rows();
        int dim = x.cols();
            ...
            for (int iter = 0; iter < maxIter; ++iter) {
                for (int iCol = 0; iCol < dim; ++iCol) {
                    for (int i = 0; i < n; ++i) {
                        ...
                        for (int j = 0; j < n; ++j) {
                            if (j != i) {
                                sum += LHS(i, j) * x(j, iCol);
                            }
                        }
                        x(i, iCol) = (rhs(i, iCol) - sum) / LHS(i, iCol);
                    }
                }
            }
        }
    ...
    void MultigridSolver::solve(Eigen::SparseMatrix<double>& LHS, Eigen::MatrixXd& rhs, Eigen::MatrixXd& x, int solverType)
    {
        ...
        if (solverType == 0) {
            ...
            if (isSmootherGaussSeidel)
            {
                ...
                //writeSparseMatrixToFile(LHS, "Abar_" + std::to_string(0));

                if (UseDense) {
                    AbarDense.resize(U.size() + 1);
                    AbarDense[1] = UDense[0].transpose() * LHS * UDense[0];

                    for (int k = 2; k < U.size() + 1; ++k) {
                        AbarDense[k] = UDense[k - 1].transpose() * AbarDense[k - 1] * UDense[k - 1];
                    }
                }
                else {
                    Abar.resize(U.size() + 1);
                    Abar[1] = U[0].transpose() * LHS * U[0];
                    //writeSparseMatrixToFile(Abar[1], "Abar_" + std::to_string(1));
                ...
                {
                    double threshold = 0.1; // set your desired threshold here

                    
                    auto construct_k_ring =
                        [&](int k, const std::vector<std::set<int>>& one_ring)
                        -> std::vector<std::set<int>>
                        {
                            const std::size_t n = one_ring.size();
                            ...
                            for (std::size_t v = 0; v < n; ++v)
                            {
                                ...
                                frontier.assign(1, static_cast<int>(v));
                                visited[v] = 1;
                                
                                for (int depth = 1; depth <= k && !frontier.empty(); ++depth) {
                                    next_frontier.clear();

                                    for (int u : frontier)
                                    {
                                        for (int nbr : one_ring[u])
                                        {
                                            if (!visited[nbr])
                                            {
                                                visited[nbr] = 1;
                    ...
                    for (int k = 1; k < Abar.size(); ++k) {
                        Eigen::SparseMatrix<double> pruned;
                        std::vector<Eigen::Triplet<double>> triplets;

                        std::vector<std::set<int>> k_ring = construct_k_ring(k + 1, mNeighborsList[k-1]);

                        for (int i = 0; i < Abar[k].outerSize(); ++i) {
                            for (Eigen::SparseMatrix<double>::InnerIterator it(Abar[k], i); it; ++it) {
                                ...
                                //only if this edge exist in the mesh at this level 
                                //if (mNeighborsList[k - 1][it.row()].find(it.col()) != mNeighborsList[k - 1][it.row()].end()) {
                                //    triplets.emplace_back(it.row(), it.col(), it.value());
                                //}

                                //prune any vertex that fall outside the k+1-ring of this level 
                                if (k_ring[it.row()].find(it.col()) != k_ring[it.row()].end()) {
                                    triplets.emplace_back(it.row(), it.col(), it.value());
                                }


                                // Strength - Of - Connection from MGPBD paper 
                                //if (std::abs(it.value()) >= 
                                //	threshold * std::sqrt(Abar[k].coeff(it.row(), it.row()) * Abar[k].coeff(it.col(), it.col())
                                //	)) {
                                //	triplets.emplace_back(it.row(), it.col(), it.value());
                                //}
                            }
                        ...
                        pruned.resize(Abar[k].rows(), Abar[k].cols());
                        pruned.setFromTriplets(triplets.begin(), triplets.end());
                        printf("\n Abar[%d] NNZ= %d, pruned NNZ= %d",
                            k, Abar[k].nonZeros(), pruned.nonZeros());
                        printf("\n Sparsity: Abar = %f, pruned= %f",
                            float(100 * Abar[k].nonZeros()) / float(Abar[k].rows() * Abar[k].cols()),
                            float(100 * pruned.nonZeros()) / float(pruned.rows() * pruned.cols()));
                ...
                if (UseDense) {
                    coarsestSolverDense.compute(AbarDense[UDense.size()]);
                }
                else {
                ...
                if (cycleType == 0) {
                    ...
                    do {
                        ...
                        if (verbose) printf("Iter= %d, Time= %f (ms), Residual= %.14f \n", ++iterCount, cyclesTimer.get_elapsed_ms(), residue);
                        else ++iterCount;
    ...
    template double MultigridSolver::multiGridVCycleGS<Eigen::SparseMatrix<double>>(Eigen::SparseMatrix<double>&, Eigen::MatrixXd&, Eigen::MatrixXd&, int, bool isDebug);
    template double MultigridSolver::multiGridVCycleGS<Eigen::MatrixXd>(Eigen::MatrixXd&, Eigen::MatrixXd&, Eigen::MatrixXd&, int, bool isDebug);
}
